...
export default function AnalysisDetail() {
  ...
  const { data: analysis } = useQuery({ queryKey:['analysis', analysisId], queryFn:()=>AnalysesAPI.get(Number(analysisId)) });
  const { data: comparisons } = useQuery({ queryKey:['comparisons', analysisId], queryFn:()=>ComparisonsAPI.listByAnalysis(Number(analysisId)) });
  ...
  return (
    <Box>
      <Typography variant="h5" gutterBottom>Análise #{analysisId}</Typography>

      <Grid container spacing={2}>
        <Grid item xs={12} lg={7}>
          <Typography variant="h6" gutterBottom>Comparações</Typography>
          <Grid container spacing={2}>
        ...
        <Grid item xs={12} lg={5}>
          <Typography variant="h6" gutterBottom>Quesitos</Typography>
          <Grid container spacing={2}>
            {(quesitos ?? []).map((q:any)=>(
